...
func RegisterFlags(flagSet *flag.FlagSet) {
	...
	flagSet.Uint64("nvproxy-set-timeslice-us", 0, "EXPERIMENTAL, operator-only: GPU scheduler timeslice, in microseconds, that nvproxy imposes on every channel group the sandbox creates. Intended to weight a sandbox's share of a contended GPU via the runlist, but measured INERT on Blackwell (the GSP scheduler ignores per-TSG timeslice for division); may work on pre-GSP GPUs. 0 disables. Not container-overridable.")
	flagSet.Uint64("nvproxy-set-interleave-level", 0, "EXPERIMENTAL, operator-only: runlist interleave level nvproxy imposes on every channel group the sandbox creates: 1=LOW, 2=MEDIUM, 3=HIGH. A coarse priority knob distinct from the timeslice, but measured admin-gated (fails INSUFFICIENT_PERMISSIONS from the unprivileged Sentry), so inoperative here. 0 leaves it alone. Not container-overridable.")
	...
	flagSet.String("amdproxy-gpu-scheduler-socket", "", "path of a `runsc gpu-scheduler` socket that divides an AMD GPU between the sandboxes sharing it. When set, the sandbox's queues are suspended outside the window the scheduler grants it. Unlike the NVIDIA side, no privileged host component is in the enforcement path: the Sentry suspends its own queues.")
	flagSet.Uint64(flagAMDProxyGPUWeight, 0, "this sandbox's share of an AMD GPU relative to the others scheduled alongside it. Only meaningful with --amdproxy-gpu-scheduler-socket, which is what turns slicing on; 0 is treated as 1 rather than leaving the sandbox unsliced. Unlike the CU mask, which partitions the device in space and cannot hand an idle tenant's share to a busy one, this divides it in time and is work-conserving.")
