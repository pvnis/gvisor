...
func TestParseCUMask(t *testing.T) {
	for _, tc := range []struct {
		...
		// Strings that begin with hexadecimal digits and then stop being
		// hexadecimal. These must be refused rather than truncated to the
		// prefix, or an operator's typo silently becomes a valid, much
		// narrower partition than the one they wrote.
		{in: "all", wantErr: true},
		{in: "0xff!", wantErr: true},
		{in: "12g4", wantErr: true},
		{in: "0xff 0xff", wantErr: true},
		// Garbage in a more significant word, which is scanned as its own
		// group and so would otherwise be truncated independently.
		{in: "0xffzzffffffff", wantErr: true},
		{in: "-1", wantErr: true},
	} {
