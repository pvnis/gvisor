...
// The parameter type is a type parameter because the inline info list grew
// between driver versions, and the driver rejects a control whose parameter
// size does not match the one it was built with. version.go selects the
// variant for the driver in use.
func ctrlFBGetInfoV2[Params any, PtrParams hasFBInfoListPtr[Params]](fi *frontendIoctlState, ioctlParams *nvgpu.NVOS54_PARAMETERS) (uintptr, error) {
